...
class ModelArguments:
    ...
    # lora训练的参数
    lora_r: Optional[int] = field(default=16, metadata={"help":"lora训练所需参数"})
    lora_alpha: Optional[int] = field(default=32, metadata={"help":"lora训练所需参数"})

    target_modules: Optional[str] = field(default="q_proj,v_proj,k_proj,o_proj,gate_proj,down_proj,up_proj", metadata={"help":"模块名称列表，或一个正则表达式，指明要使用Lora替换的模块。例子: ['q', 'v'] or '.*decoder.*(SelfAttention|EncDecAttention).*(q|v)$'"})
