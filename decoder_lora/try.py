...
def try2():
    ...
    adapter_model_name_or_path = './output/v4/best_lora_model'
    ...
    input_ids = tokenizer(["<s>Human: 给出一条规则，请你结合提供的领域知识尽可能全面地将规则中的关键信息抽取出来，并将抽取的信息组装成结构化语句。\n规则:3.1.6 采用点击成交、询价成交和协商成交方式的，交易时间为每个交易日的9:00至11:30、13:00至20:00。\n领域知识：交易方式:['匹配成交', '点击成交', '询价成交', '竞买成交', '协商成交']\n</s><s>Assistant:"], return_tensors="pt", add_special_tokens=False).input_ids
