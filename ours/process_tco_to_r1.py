...
def read_model_output_to_rule(texts, labels):
    """
    读取模型输出的内容，将其转化为key-value对的形式，存放在stack中。同时记录句子中的；和。并记录在sentence_separate_1和sentence_separate_2中
    ...
    operator_count = 0
    stack_index = 0
    if list(stack[stack_index].keys())[0] == "操作主体":
        operator_count += 1
    for i, text in enumerate(texts):
        if stack_index+1 < len(stack) and list(stack[stack_index].values())[0] in texts[:i+1]:
            stack_index += 1
            if list(stack[stack_index].keys())[0] == "操作主体":
                operator_count += 1
                if operator_count >= 2:
                    stack_value = list(stack[stack_index].values())[0]
                    if texts[i-1-len(stack_value)] == "向" or texts[i-1-len(stack_value)] == "给" or (i-2-len(stack_value) >= 0 and texts[i-2-len(stack_value):i-len(stack_value)] == "委托"):
                        operator_relation.append(1)
                    elif texts[i-1-len(stack_value)] == "与":
                        operator_relation.append(2)
                    elif (i-2-len(stack_value) >= 0 and texts[i-2-len(stack_value):i-len(stack_value)] == "作为"):
                        operator_relation.append(3)
                    elif texts[i-1-len(stack_value)] == "，":
                        operator_relation.append(0)
        
        if text == "；":
            sentence_separate_1.append(stack_index)
        elif text == "。":
            sentence_separate_2.append(stack_index)
        elif text == "，":
            sentence_separate_3.append(stack_index)
        elif texts[i] in ["且", "但"]:
            sentence_and.append(stack_index)
...
def to_r1_v2(rules, knowledge, terms):
    ...
    for rule in rules:
        ...
        stack, sentence_separate_1, sentence_separate_2, sentence_separate_3, sentence_and, operator_relation = read_model_output_to_rule(texts, labels)
